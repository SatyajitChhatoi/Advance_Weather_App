...
function dayOfTheWeek(day, month, year) {
  ...
  const date = new Date(`${formattedMonth}/${formattedDay}/${year}`);
...
function fetchesWeatherData() {
  // Fetches the data and dynamically add the city name with template literals
  // USE YOUR OWN KEY
  fetch(
    `http://api.weatherapi.com/v1/current.json?key=b992dea55878467c8f952456232805&q=${cityInput}`
  )
    ...
    .then((data) => {
      ...
      icon.src = "./weather/64x64/icons/" + iconId;
      ...
      if (code == 1000) {
        ...
        // Change the button background color depending on if it's day or night
